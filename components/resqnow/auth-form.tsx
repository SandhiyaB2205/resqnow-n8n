...
export default function AuthForm({ signup = false }: { signup?: boolean }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const result = signup ? await authClient.signUp.email({ email: values.email, password: values.password, name: values.name }) : await authClient.signIn.email({ email: values.email, password: values.password })
    setLoading(false)
