...
export default function AuthForm({ signup = false }: { signup?: boolean }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const result = signup ? await authClient.signUp.email({ email: values.email, password: values.password, name: values.name } as Parameters<typeof authClient.signUp.email>[0]) : await authClient.signIn.email({ email: values.email, password: values.password })
